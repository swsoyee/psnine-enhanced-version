...
(function () {
    ...
    repeatUntilSuccessful(() => {// MutationOvserver won't start until DOMContentLoaded
        if (!document.body)
            return false;
        onBodyExistence();
        return true;
    }, 5);// 200 Hz, should start fast enough to eliminate flashing
    document.addEventListener("DOMContentLoaded", onDOMContentReady);

    // 全局优化
    function onBodyExistence() {// run as soon as body element exists
        // 功能0-2：夜间模式
    ...
    function onDOMContentReady() {
        ...
        const addHotTag = () => {
            $('div.meta').map((index, element) => {
                ...
                if (Number(replyCount[replyCount.length - 2]) > 15
        ...
        const changeQaStatus = (isOn) => {
            if (isOn) {
                // 替换文字状态为图标形式
                $('div.meta > .r > span').map((i, el) => {
                    const status = $(el).text();
                    // 替换文字状态为图标形式
                    if (status === "已解决") {
                        $(el).replaceWith('<div class="fa-check-circle"></div>');
                    } else if (status === "未回答") {
                        $(el).replaceWith('<div class="fa-question-circle"></div>');
                    } else if (status === "解决中") {
                        $(el).replaceWith('<div class="fa-comments"></div>');
                    } else {
                        // 替换文字状态为图标形式
                        const reward = status.match(/悬赏(\d+)铜/);
                        if (reward && reward.length > 0) {
                            const number = Number(reward[1]);
                            $(el).replaceWith(`<div class="fa-coins"></div>&nbsp;<span class="${number > 30 ? 'text-gold' : (number === 10 ? 'text-bronze' : 'text-silver')}" style="font-weight:bold;"}">${number}</span>`);
                        }
                    }
                });
            } else {
